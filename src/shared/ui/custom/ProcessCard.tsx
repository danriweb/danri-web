...
export const ProcessCard = ({ id, title, description, icon: Icon, index, className }: ProcessCardProps) => {
  return (
    <m.div
      ...
      className={cn("group relative p-px overflow-hidden rounded-[12px] bg-white/5", className)}
    >
      ...
      <m.div
        ...
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 aspect-square w-[200%] opacity-20 transition-opacity duration-500 group-hover:opacity-100"
      />

      <div
        className={cn(
          "relative z-10 flex h-full w-full cursor-pointer select-none flex-col gap-6 rounded-[11px] bg-card p-8 transition-all duration-500",
        )}
      >
        {/* Градиенты фона */}
        ...
        <div className="relative z-10 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-col gap-1">
              <span className="text-muted-foreground text-xs font-bold tracking-wider uppercase opacity-50">{id}</span>
              <h3 className="text-xl font-bold tracking-tight text-white uppercase">{title}</h3>
            </div>
